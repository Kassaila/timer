...
В параметрах вывода сообщений по умолчанию установлена пустая строка.
```html
<div id="timer_block">
...
```
### Callback  
Для состояния по окончанию **Timer** или на период паузы (зависит от установленного типа **Timer**) будет вызвана функция `timerCallback()` с телом функции переданным в неё.
```javascript
timer({
    idTimer: '#timer_block',
    deadLine: {
...
```
В данном примере по окончанию **Timer** будет выведено сообщение в консоль.